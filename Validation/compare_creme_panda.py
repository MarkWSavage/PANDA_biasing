...
#
# Uses the DEPOSITED-charge cross section, not collected. CREME-MC (and
# CUPID) assume the RPP model's 100%-charge-collection convention --
# every e-h pair generated by the deposited energy is assumed to reach
# the electrode. PANDA's "collected" curve additionally applies a
# drift/trapping collection-efficiency model (T = exp(-tau_d/tau_r) < 1,
# see SteppingAction.cc), which CREME-MC has no equivalent of. Comparing
# CREME-MC against PANDA's collected curve compares two different
# physical quantities and would show PANDA as systematically "softer"
# than it actually is relative to the RPP assumption CREME-MC shares.
script_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(script_dir)
panda_file = os.path.join(
    project_root, "Results", "Current", "cumulative_cross_section_deposited.csv"
)
