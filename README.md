...
**Current release: [v1.7.13](https://github.com/MarkWSavage/PANDA/releases/tag/v1.7.13) — Au197 Low-Energy Straggling Mechanism Resolved.** Closes out the open question left by v1.7.11's straggling finding (200 MeV Au197's anomalously narrow differential charge spectrum vs. a properly-spread peak at 3000 MeV): the earlier writeup compared SRIM dE/dx (roughly comparable at both energies) and couldn't explain the ~2.8x difference in relative spread. The actual driver is the Vavilov distribution shape parameter kappa = xi/W_max ~ z^2/beta^4 -- a steep inverse-fourth-power *velocity* dependence, not a dE/dx dependence. For Au197, beta=0.0467 at 200 MeV vs. beta=0.1786 at 3000 MeV gives kappa_200/kappa_3000 ~ 215x, putting 200 MeV far deeper into the Gaussian (many-effective-collisions) regime -- correctly predicting both the direction and magnitude of the observed narrowing. Confirms the mechanism as ordinary Landau/Vavilov collision-counting straggling (per Weeden-Wright et al. 2015), not charge-state fluctuation. Documentation only -- no functional or behavioral code changes. See `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 2 for the full derivation.
